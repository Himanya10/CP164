...
class ByLetter:
    ...
    def __eq__(self, target):
        ...
        return isinstance(target, ByLetter) and self.letter == target.letter
    ...
    def __lt__(self, target):
        ...
        return isinstance(target, ByLetter) and (self.letter < target.letter if target is not None else False)
...
class ByCode:
    ...
    def __eq__(self, target):
        ...
        return isinstance(target, ByCode) and self.code == target.code
    ...
    def __lt__(self, target):
        ...
        -------------------------------------------------------
        """
        return isinstance(target, ByCode) and (self.code < target.code if target is not None else False)
...
def encode_morse(bst, text):
    ...
    for char in text.upper():
        if char == ' ':
            ...
            by_letter = ByLetter(char, '')
            node = bst.retrieve(by_letter)
            if node is not None:
                result += node.code + ' '
...
def decode_morse(bst, code):
    ...
    for morse_code in morse_chars:
        if morse_code == '':
            result += ' '
        else:
            by_code = ByCode('', morse_code)
            node = bst.retrieve(by_code)
            if node is not None:
                result += node.letter
