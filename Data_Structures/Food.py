...
class Food:
    ...
    def origins():
        ...
        -------------------------------------------------------
        """
        max_width = len(str(len(Food.ORIGIN) - 1))  
        result = "\n".join(f"{i:>{max_width}} {origin}" for i, origin in enumerate(Food.ORIGIN))
        return result
    ...
    def __str__(self):
        ...
        Vegetarian_str = "Vegetarian" if self.is_vegetarian else "Non-Vegetrian"
        calories_str = str(self.calories) if self.calories is not None else "Not specified"
        formatted_string = f"Name: {self.name}\n" \
                        f"Origin: {origin_str}\n" \
                        f"Vegetarian: {Vegetarian_str}\n" \
                        f"Calories: {calories_str}"
